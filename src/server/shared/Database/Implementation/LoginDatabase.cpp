...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    PREPARE_STATEMENT(LOGIN_DEL_ACCOUNT_IP_HISTORY, "DELETE FROM account_ip_history WHERE id = ? AND ip = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_INS_ACCOUNT_IP_HISTORY, "INSERT INTO account_ip_history (id,ip,last_access) VALUES (?, ?, CURRENT_TIMESTAMP())", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_SEL_ACCOUNT_IP_HISTORY_BY_ACCOUNT, "SELECT ip, last_access FROM account_ip_history WHERE id = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(LOGIN_SEL_ACCOUNT_IP_HISTORY_BY_IP, "SELECT id, last_access FROM account_ip_history WHERE ip = ?", CONNECTION_SYNCH);
}
